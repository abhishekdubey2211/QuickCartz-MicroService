...
public class RedisConn {
	...
	private int nType = 0;
	private ServerDetails objRedisServerDetails = null;

//    public RedisConn(ServerDetails redisServerDetails) {
//        if (redisServerDetails == null) {
//            throw new IllegalArgumentException("Redis server details cannot be null");
//        }
//        this.objRedisServerDetails = redisServerDetails;
//    }

	public void initializeRedis() {
		log.info("******* Initializing Redis ******* Version: {}", ConstantData.strRedisVersion);

		if (objRedisServerDetails == null) {
			objRedisServerDetails = new ServerDetails();
			objRedisServerDetails.setIpAddress("localhost");
			objRedisServerDetails.setPortno(6379);
			objRedisServerDetails.setConnectiontype(1);
		}
		...
		try {
			...
			case ConstantData.TYPE_SENTINAL:
				...
				objSentinelPool = new JedisSentinelPool(objRedisServerDetails.getLoginName(), sentinels, objConfig,
						ntimeout, objRedisServerDetails.getPassword());
				log.info("Initialized Redis with sentinels.");
				break;
			case ConstantData.TYPE_SINGLE:
				objSinglePool = new JedisPool(objConfig, objRedisServerDetails.getIpAddress(),
						objRedisServerDetails.getPortno(), ntimeout, objRedisServerDetails.getPassword());
			...
			case ConstantData.TYPE_SINGLE_SECURE:
				...
				objSinglePool = new JedisPool(objConfig, objRedisServerDetails.getIpAddress(),
						objRedisServerDetails.getPortno(), ntimeout, objRedisServerDetails.getPassword(), true);
	    ...
	    try {
	        if (nType == ConstantData.TYPE_SENTINAL || nType == ConstantData.TYPE_SENTINAL_SECURE) {
	            if (objSentinelPool == null) {
	                log.error("Sentinel Pool is NULL before initialization");
	                initializeRedis();
	            }
	            if (objSentinelPool == null) {
	                log.error("Sentinel Pool is still NULL after initialization");
	                return null;
	            }
	            return objSentinelPool.getResource();
	        } else if (nType == ConstantData.TYPE_SINGLE || nType == ConstantData.TYPE_SINGLE_SECURE) {
	            if (objSinglePool == null) {
	                log.error("Single Pool is NULL before initialization");
	                initializeRedis();
	            }
	            if (objSinglePool == null) {
	                log.error("Single Pool is still NULL after initialization");
	                return null;
	            }
